    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-100 via-white to-white text-slate-800">
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-8 items-start">
          <div className="md:col-span-2">
            ...
            <p className="mt-4 text-slate-700 leading-relaxed">
              ...
              Project Management Professional (PMP)® 資格保有。
            </p>
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-8 items-center">
          <div className="md:col-span-2">
            ...
            <p className="mt-3 text-slate-600">
              オンライン相談（無料）を承ります。下記からご連絡ください。
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="mailto:takamichi.kojima@kjbiz.jp" className="soft-btn-primary">
                <Mail className="size-4" /> メール
              </a>
              {/* Microsoft Bookings: 予約フォームへ 
              <a
                href="https://outlook.office365.com/owa/calendar/Meetings@kjbiz.jp/bookings/" 
                target="_blank"
                rel="noopener noreferrer"
                className="soft-btn-ghost flex items-center gap-2"
                aria-label="Microsoft Bookingsで予約"
              >
                <CalendarIcon className="size-4" /> 相談枠を予約
              </a>*/}
              <a
      ...
      <footer className="py-10 border-t bg-white/60">
        <div className="max-w-6xl mx-auto px-4 text-sm text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>© {new Date().getFullYear()} KOJIMA WORKS</div>
...
function MacWindow({ children }: { children: React.ReactNode }) {
  return (
    <div className="soft-card overflow-hidden">
      <div
        className="h-9 flex items-center gap-1 px-3 border-b bg-white/60 backdrop-blur"
...
function TechScroller({ items }: { items: string[] }) {
  ...
  return (
    <div className="relative mt-4">
      ...
      >
        <div className="flex gap-2 py-1 pr-8 min-w-max">
          {items.map((t, i) => (
            <span
              key={t}
              data-chip={i === 0 ? "1" : "0"}
      ...
      <button
        ...
        onClick={() => slide(-1)}
        disabled={!canL}
        className="absolute left-0 top-1/2 -translate-y-1/2 translate-x-[-6px] size-8 grid place-items-center rounded-full border bg-white shadow disabled:opacity-30"
        style={{ borderColor: "var(--border)" }}
      >
        ‹
      </button>
      <button
        ...
        onClick={() => slide(1)}
        disabled={!canR}
        className="absolute right-0 top-1/2 -translate-y-1/2 -translate-x-[-6px] size-8 grid place-items-center rounded-full border bg-white shadow disabled:opacity-30"
        style={{ borderColor: "var(--border)" }}
      >
        ›
      </button>
